...
class CustomTokenObtainPairView(TokenObtainPairView):
    def post(self, request, *args, **kwargs):
        ...
        # Set secure HTTP-only cookies with settings that match JWT token lifetimes
        if access:
            response.set_cookie(
                key='access_token',
                value=access,
                ...
                max_age=settings.ACCESS_TOKEN_COOKIE_AGE,         # Matches JWT token lifetime
        ...
        if refresh:
            response.set_cookie(
                ...
                samesite=settings.COOKIE_SAMESITE,                # CSRF protection
                secure=settings.COOKIE_SECURE,                    # HTTPS only in production
                max_age=settings.REFRESH_TOKEN_COOKIE_AGE,        # Matches JWT token lifetime
                path='/api/',                                     # Only sent to API endpoints
            )
